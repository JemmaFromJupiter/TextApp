	...
	def mainloop(self):
		while True:
			...
			if event in [sg.WINDOW_CLOSED, "Exit"]:
				break
			if event == '-BGCOL-':
				self.window["-BGCOL_-"].update(background_color=values["-BGCOL-"])
			if event == '-TXTCOL-':
				self.window["-TXTCOL_-"].update(background_color=values["-TXTCOL-"])
			if event == '-INCOL-':
				self.window["-INCOL_-"].update(background_color=values["-INCOL-"])
			if event == '-TXINCOL-':
				self.window["-TXINCOL_-"].update(background_color=values["-TXINCOL-"])
			if event == '-SCRLCOL-':
				self.window["-SCRLCOL_-"].update(background_color=values["-SCRLCOL-"])
			if event == "-BTNCOL-":
				self.window["-BTNCOL1_-"].update(background_color=values["-BTNCOL-"])
			if event == "-BTNCOL2-":
				self.window["-BTNCOL2_-"].update(background_color=values["-BTNCOL2-"])
			if event == "-PRGCOL-":
				self.window["-PRGCOL1_-"].update(background_color=values["-PRGCOL-"])
			if event == "-PRGCOL2-":
				self.window["-PRGCOL2_-"].update(background_color=values["-PRGCOL2-"])
...
class MainApp(App):
	...
	def create_window_bindings(self):
		...
		self.window.bind('<Control_L><Shift_L><Z>', lambda text=self.text:clipboard_funcs.redo(text))
		self.window.bind('<Control_L><o>', "Open File")
		self.window.bind('<Control_L><s>', "Save File")
		self.window.bind('<Control_L><Shift_L><S>', "Save As")
		self.window.bind('<Control_L><Shift_L><R>', "Run Code")
	...
	def configure_lineNums(self):
		...
		self.repack(self.m1.widget.master, {'fill':'y', 'expand':False, 'before':self.m2.widget.master})
